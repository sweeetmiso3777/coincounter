...
async function fetchSalesWithCacheFallback(todayKey: string): Promise<SalesDocument[]> {
  ...
  try {
    ...
    console.log(`Server fetch: ${serverSnapshot.docs.length} documents retrieved`)
  ...
  useEffect(() => {
    ...
    const setupRealTimeListener = async () => {
      if (!queryResult.data || queryResult.data.length === 0) {
        console.log(` No data available for real-time listener setup`)
        return
      }

      realTimeListenerSetup.current = true
      console.log(`Setting up real-time listener with ${queryResult.data.length} existing items`)

      // Get the latest timestamp from cached data
      const latest = queryResult.data[0]
      if (!latest?.timestamp) {
        console.warn(`No timestamp found in latest document`)
        return
      }
      ...
      try {
        if (latest.timestamp && typeof latest.timestamp === 'object' && 'toDate' in latest.timestamp) {
          ...
          console.warn(`Unable to parse timestamp:`, latest.timestamp)
          return
        }

        console.log(`Real-time listener watching for sales after:`, latestTimestamp)
          ...
          (snapshot) => {
            ...
            if (fresh.length > 0) {
              ...
              queryClient.setQueryData<SalesDocument[]>([...CACHE_KEY, todayKey], (old) => {
                ...
                console.log(`Cache updated: ${old?.length || 0} → ${updated.length} items`)
                return updated
              })
              ...
              console.log(`Real-time listener triggered but no new sales found`)
            }
          },
          (error) => {
