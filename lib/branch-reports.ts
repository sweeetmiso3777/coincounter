...
export function generateBranchHarvestPDF(
  ...
  if (result.summary.branchSharePercentage && result.summary.branchSharePercentage > 0) {
    ...
    const shareData = [
      ...
      { label: 'Your Share Amount:', value: `P ${result.summary.branchShareAmount?.toLocaleString('en-PH', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` },
      { label: 'Company Share Amount:', value: `P ${result.summary.companyShareAmount?.toLocaleString('en-PH', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` },
    ];
  ...
  const metricsData = [
    ...
    { label: 'Aggregates Harvested:', value: result.summary.aggregatesHarvested.toLocaleString() },
  ];
  ...
  doc.text(`Report generated by Coin Harvest System - Page ${currentPage}`, pageWidth / 2, pageHeight - 10, { align: 'center' });
  ...
  // Monthly Progress Section - MOVED TO PAGE 2
  if (result.monthlyAggregate) {
    ...
    const monthlyData = [
      ...
      { 
        label: 'Month to Date:', 
        value: `P${result.monthlyAggregate.total.toLocaleString('en-PH', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` 
      },
      { 
        label: 'Monthly Transactions:', 
        value: result.monthlyAggregate.sales_count.toLocaleString() 
      },
    ];
  ...
  const performanceData = [
    { label: 'Average per Unit:', value: `P${avgPerUnit.toLocaleString('en-PH', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` },
  ...
  if (result.monthlyAggregate?.unit_summaries && result.monthlyAggregate.unit_summaries.length > 0) {
    ...
    const tableHeaders = ['Unit', 'Summaries', 'Sales Count', 'Total Amount', 'P1 Coins', 'P5 Coins', 'P10 Coins', 'P20 Coins'];
  ...
  doc.setFontSize(8);
  doc.setTextColor(150, 150, 150);
  doc.text(`Report generated by Coin Harvest System - Page ${currentPage} of ${finalTotalPages}`, pageWidth / 2, finalPageHeight - 10, { align: 'center' });
...
export function generateCompactBranchHarvestPDF(
  ...
  doc.setFontSize(16);
  doc.setTextColor(0, 128, 0);
  doc.text(`EXPECTED REVENUE: P ${result.summary.totalAmount.toLocaleString('en-PH', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`, pageWidth / 2, yPosition, { align: 'center' });
  ...
  if (result.summary.branchSharePercentage && result.summary.branchSharePercentage > 0) {
    ...
    const shareData = [
      { label: 'Your Share:', value: `P ${result.summary.branchShareAmount?.toLocaleString('en-PH', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} (${result.summary.branchSharePercentage}%)` },
    ];
